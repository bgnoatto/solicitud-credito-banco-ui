...
VERSION=5.1.0
USAGE="Uso:\t./dockerizar-nodejs-angular.sh"

# ------------------------------------------------------------------

regla='^[a-z0-9_-]{5,}:v[0-9]+\.[0-9]+\.?[0-9]*$'
reglaPuerto='^82[0-9]{2}$'
...
echo "COPY dist /usr/share/nginx/html" >> $rutaDockerfile/Dockerfile && echo '' >> $rutaDockerfile/Dockerfile
